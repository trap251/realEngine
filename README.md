...
2. Learn "Creating a window" at learnopengl.com. Make a WindowHandling class for the GLFW window handling functionality. Files: WindowHandling.h, WindowHandling.cpp
...
5. GLFW has input handling functionality. Have an Input class and implement GLFW Input Handling within it, follow information hiding principles. Files: Input.h, Input.cpp
6. Implement shader handling in API/ as it involves API-specific functions. 
7. Include a maths library and implement Transform(Scale, Rotate, Translate) functionality. Add helper functions to the Shader namespace to pass values such as int,float,vec3, mat4, etc. to shaders. There is always an API function for that purpose. 
8. Understand Euclidean angles and view, model, and projection matrices. Implement a 'Camera' using these. Camera and Input need to communicate, use Backend to pass pointers to functions instead of intermingling these components. For example, a ProcessMouseMovement function from Camera can be passed to Input::Init() inside Backend. Input::Init() can use it as void (*processMouseMovementFunc_camera). This is how you can pass functions as parameters to other functions. Useful in keeping components separate and engine architecture clean.
